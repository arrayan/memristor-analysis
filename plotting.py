...
# ------------------------------------------------------------------
DB_FILE = Path(
    r"C:\Users\apesc\OneDrive\Dokumente\TU_SEM1\MEMRISTOR"
...
yaxis_ai = dict(
    type="log",
    tickmode="array",
    tickvals=10.0 ** np.arange(-12, 0),
    ticktext=[f"1×10<sup>{int(np.log10(d))}</sup>" for d in 10.0 ** np.arange(-12, 0)],
    title="AI (A)",
)
yaxis_norm = dict(
    type="linear",
...
V_forming_global = (
    forming_df["V_forming_global"].iloc[0] if not forming_df.empty else None
)
...
for s in sets:
    ...
    vreset_s = (
        df_s.groupby("cycle_number")[["AV", "AI"]]
        .apply(lambda g: g.loc[g["AI"].abs().idxmax(), "AV"], include_groups=False)
...
for param, info in param_map.items():
    for s, sc in zip(sets, set_colors):
        vals = (
            cdf_full[cdf_full["source_file"] == s][param]
            .dropna()
            .sort_values()
            .to_numpy()
        )
...
def build_vis(param, set_val):
    return [
        tr.meta["param"] == param and tr.meta["set"] == set_val for tr in cdf_fig.data
    ]
...
box_data = classic_box.merge(
    vreset_box_df, on=["source_file", "cycle_number"], how="left"
).merge(ireset_box_df, on=["source_file", "cycle_number"], how="left")
...
for param, info in param_map.items():
    ...
    buttons.append(
        dict(
            ...
            args=[
                {"visible": visible},
                {"yaxis": yax, "title": f"Boxplot – {info['pretty']}"},
            ],
        )
    )
...
scatter_df = end_df[
    ["source_file", "cycle_number", "V_set", "V_reset", "I_HRS", "I_LRS", "I_reset_max"]
].copy()
...
def vis(set_name, x_y):
    return [
        tr.meta["set"] == set_name and tr.meta["x"] == x_y[0] and tr.meta["y"] == x_y[1]
...
for x_col, y_col, title in pairs:
    visible = [
        tr.meta["x"] == x_col and tr.meta["y"] == y_col for tr in fig_scatter.data
    ]
    pair_buttons.append(
        dict(
            label=title,
...
for s in sets:
    visible = [
        tr.meta["set"] == s and tr.meta["x"] == fig_scatter.layout.xaxis.title.text
        for tr in fig_scatter.data
    ]
    set_buttons.append(
        dict(
